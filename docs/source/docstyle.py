...
class PlainDefinition(Item):
    """ A Definition parser that treats anything after the ``:`` as a single classifier
...
def returns_section(doc, header, renderer=None, item_class=None):
    """ A return item list section that also accepts a paragraph as a description.
...
def function_section(lines):
    return DocRender(
        ...
        sections={
            ...
            'Arguments': (arguments, Argument, PlainDefinition),
            'Parameters': (arguments, Argument, PlainDefinition),
            'Raises': (item_list, ListItem, PlainDefinition),
